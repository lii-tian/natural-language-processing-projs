...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = np.matmul(outsideVectors.T, y_hat - y)  #(d, v) * (v, 1) = (d,)

    gradOutsideVecs = np.matmul((y_hat - y).reshape((-1,1)), centerWordVec.reshape((1,-1))) #(v, 1) * (1 , d)= (v,d)
...
def negSamplingLossAndGradient(
    ...
    all_vec = outsideVectors[indices]   # select ousideVec + neg sample vectors, [1+k, d]
    all_vec[1:] *= -1                    # negate the neg samples vectors

    temp = sigmoid(np.matmul(all_vec, centerWordVec)) # [1+k, d] x[d,] = [1+k,] 

    loss = -np.sum(np.log(temp))

    gradCenterVec = np.matmul(all_vec.T, temp - 1)    #[d, 1+k] x [1+k,] = [d,]
    
    gradOutsideVecs = np.zeros_like(outsideVectors)  #initialize the gradient
    grad= np.matmul((temp- 1).reshape(-1,1), centerWordVec.reshape(1,-1))   #[1+k,1] x [1,d] = [1+k,d]
    grad[1:] *= -1
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centerWordVec = centerWordVectors[centerWordIdx]  # (1, d)
    outsideWordIndices = [word2Ind[i] for i in outsideWords] 

    for outsideWordIdx in outsideWordIndices:
        loss_, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        
        loss += loss_ #scalar
        gradCenterVecs[centerWordIdx] += gradCenterVec.reshape(-1)  #[1,d]
