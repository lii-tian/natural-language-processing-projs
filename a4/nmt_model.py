...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        src_len = source_padded.shape[0]    #maximum source sentence length, 
        b = source_padded.shape[1]          #batch size

        X = self.model_embeddings.source(source_padded)                                                     # (src_len, b, e)
        X = self.post_embed_cnn(X.permute((1,2,0))).permute((2,0,1))                                      # permute X to (b, e, src_len), send to cnn,  # permute X back to (src_len, b, e)
        enc_hiddens,  (last_hidden, last_cell) = self.encoder(pack_padded_sequence(X, source_lengths))        #feed through encoder, (src_len, b, h*2)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)                                                    # apply pad_packed_sequence, (src_len, b, h*2)
        enc_hiddens= enc_hiddens.permute((1,0,2))                                                              #(b, src_len, h*2)

        init_decoder_hidden = self.h_projection(torch.cat((last_hidden[0], last_hidden[1]), dim = 1))          # (b, 2h)
        init_decoder_cell = self.c_projection(torch.cat((last_cell[0], last_cell[1]), dim = 1))                # (b, 2h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(dec_hidden.unsqueeze(1), enc_hiddens_proj.transpose(2,1)).squeeze(1)   # (b,1,h) x (b, h, m) -> (b, 1, m) -> (b,m)
        ...
        ### END YOUR CODE
        alpha_t = F.softmax(e_t, dim = 1)   #(b,m)
        a_t = torch.bmm(alpha_t.unsqueeze(1) , enc_hiddens).squeeze(1)   #(b,1,m) x (b, m, 2h) -> (b ,1 ,2h) -> (b,2h)
